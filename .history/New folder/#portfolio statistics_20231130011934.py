...
returns= pd.DataFrame(columns=["CAN","US","INT","ALT","OB"],data=np.array([np.sum(ReturnSPTSX), np.sum(sp_returns), np.sum(ReturnsXINCad), np.sum(ReturnsaLT), np.sum(ReturnsXBB)]).T)
