...
returns = pd.DataFrame(columns=["CAN","US","INT","ALT","OB"], data=[ReturnSPTSX, sp_returns, ReturnsXINCad, ReturnsaLT, ReturnsXBB])



returns= pd.DataFrame(columns=["CAN","US","INT","ALT","OB"],data=[np.sum(ReturnSPTSX), np.sum(sp_returns), np.sum(ReturnsXINCad), np.sum(ReturnsaLT), np.sum(ReturnsXBB)])
