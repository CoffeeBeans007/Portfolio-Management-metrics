...
PF_Ref_df["Total_Returns"]=returnss
...
historic_var = np.percentile(PF_Ref_df["Total_Returns"], 5)
print(f"Historic VaR($): {historic_var} Dollars")

#Beta of PF
Beta_PF = np.cov(PF_Ref_df["Total_Returns"], index_returns)[0, 1] / np.var(index_returns)
...
#Replication Error
Replication_Error = np.std(PF_Ref_df["Total_Returns"]-index_returns[1:])
print("Replication Error: ", Replication_Error)

#Information Ratio
IR = (np.sum(PF_Ref_df["Total_Returns"]-index_returns[1:]))/Replication_Error
